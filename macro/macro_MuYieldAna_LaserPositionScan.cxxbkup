...
void macro_MuYieldAna_LaserPositionScan2(){//TString filename = "MuYield.root", int MCtype=1002){

	const int Nfile = 2;//4;//8;
...
*/
	...
	for(int i = 0; i< Nfile; i++){
		...
		for(int j = 2; j<24; j++){
			TString savefilename = filename[i] + Form("WithReflection_50055006_%1d",j); //("TBEAM_0",Form("TBEAM_0_%d",j) )
			t[i]->Init(t[i]->tree);
			t[i]->Surface();
	...
	tl->AddEntry(g_YieldWithLaserPosition[0],"itv = 6 mm","l");
	tl->AddEntry(g_YieldWithLaserPosition[1],"itv = 7 mm","l");
	...
	tl->Draw();
	c0->SaveAs("210207-itvScan-only50055006.C");
	c0->SaveAs("210207-itvScan-only50055006.pdf");
	c0->SaveAs("210207-itvScan-only50055006.root");
